...
#     self.ws.send(self.freq)
 #    self.serversocket.sendall(str(self.freq).encode('UTF-8'))
     ...
     i=self.readyFlag
   # Always finish with direction=1 (high-side injection) so HLSI bit stays
   # consistent. Alternating this bit causes the chip to retune to a slightly
   # different internal offset, which is what makes the audio sound rough.
   self.writeFrequency(self.freq, 0, 1)
